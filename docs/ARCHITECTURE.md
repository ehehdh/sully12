# 🏗️ 시스템 아키텍처

> Politi-Log의 시스템 설계 및 아키텍처 문서

## 📊 현재 상태 요약

| 항목 | 현재 | 상태 |
|------|------|------|
| **아키텍처** | 단일 Next.js 앱 | ✅ |
| **상태 관리** | Zustand + Supabase | ✅ |
| **AI 프롬프트** | 외부 설정 파일 | ✅ |
| **인증** | 카카오 OAuth | ✅ |
| **마이페이지** | 통계/전적 시스템 | ✅ |
| **토론 기록** | 자동 아카이브 | ✅ |
| **관리자** | 이슈/토론 관리 | ✅ |

---

## 🎯 아키텍처 다이어그램

```
┌─────────────────────────────────────────────────────────┐
│                      Client (Next.js)                    │
├─────────────────────────────────────────────────────────┤
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌─────────┐ │
│  │   Pages   │  │Components│  │  Hooks   │  │  Store  │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬────┘ │
│       └─────────────┴─────────────┴─────────────┘       │
├─────────────────────────────────────────────────────────┤
│                     API Routes                           │
│  ┌──────────┐  ┌──────────┐  ┌──────────┐  ┌─────────┐ │
│  │  /auth   │  │ /debates │  │/realtime │  │ /users  │ │
│  └────┬─────┘  └────┬─────┘  └────┬─────┘  └────┬────┘ │
└───────┴─────────────┴─────────────┴─────────────┴───────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────┐
│                    External Services                     │
├────────────────┬────────────────┬───────────────────────┤
│    Supabase    │   Groq API     │    Kakao OAuth        │
│  ┌───────────┐ │ ┌────────────┐ │  ┌─────────────────┐  │
│  │PostgreSQL │ │ │ Llama 3.1  │ │  │ 카카오 로그인   │  │
│  │- rooms    │ │ │- Analysis  │ │  │ - 사용자 인증   │  │
│  │- messages │ │ │- Verdict   │ │  │ - 프로필 정보   │  │
│  │- users    │ │ └────────────┘ │  └─────────────────┘  │
│  │- records  │ │                │                       │
│  └───────────┘ │                │                       │
└────────────────┴────────────────┴───────────────────────┘
```

---

## 📁 코드 구조

```
src/
├── app/                          # Next.js App Router
│   ├── page.tsx                  # 홈페이지
│   ├── login/                    # 로그인 페이지
│   ├── mypage/                   # 마이페이지 🆕
│   │   ├── page.tsx              # 프로필/통계
│   │   └── history/              # 토론 기록
│   ├── admin/                    # 관리자 페이지
│   │   ├── page.tsx              # 이슈 관리
│   │   └── debates/              # 토론 기록 관리 🆕
│   ├── debate/[id]/              # 토론방 페이지
│   ├── lobby/                    # 로비
│   └── api/                      # API 라우트
│       ├── auth/                 # 인증 API 🆕
│       │   ├── kakao/            # 카카오 로그인
│       │   ├── me/               # 현재 사용자
│       │   └── logout/           # 로그아웃
│       ├── debates/              # 토론 기록 API 🆕
│       │   ├── archive/          # 아카이브
│       │   └── records/          # 기록 조회
│       ├── users/                # 사용자 API 🆕
│       │   └── me/
│       │       ├── stats/        # 통계
│       │       └── history/      # 전적
│       ├── rooms/                # 방 관련 API
│       ├── realtime/             # 실시간 API
│       └── topics/               # 주제 API
│
├── components/                   # UI 컴포넌트
│   ├── ui/                       # 기본 UI (버튼, 카드 등)
│   ├── auth/                     # 인증 컴포넌트 🆕
│   │   ├── KakaoLoginButton.tsx
│   │   └── UserAuthButton.tsx
│   └── debate/                   # 토론 관련 컴포넌트
│
├── config/                       # 설정 파일
│   └── prompts/                  # AI 프롬프트
│
├── features/                     # 기능 모듈 (블록)
│   ├── auth/                     # 인증 모듈
│   ├── admin/                    # 관리자 모듈
│   ├── debate/                   # 토론 핵심 모듈
│   └── ai-moderator/             # AI 중재자 모듈
│
└── lib/                          # 유틸리티
    ├── supabase.ts               # Supabase 클라이언트
    ├── useAuth.tsx               # 인증 훅 🆕
    ├── db.ts                     # DB 함수
    └── debateStages.ts           # 토론 단계 설정
```

---

## 🗄️ 데이터베이스 스키마

### 핵심 테이블

```sql
-- 사용자 (카카오 로그인)
users (
  id, kakao_id, email, nickname, profile_image, role, created_at
)

-- 토론 기록 (아카이브)
debate_records (
  id, topic_id, topic_title,
  pro_user_id, pro_user_name,
  con_user_id, con_user_name,
  winner, final_score_pro, final_score_con,
  verdict_summary, total_messages, duration_seconds,
  started_at, ended_at
)

-- 토론 메시지 아카이브
debate_messages_archive (
  id, debate_record_id,
  role, content, sender_name, stage,
  original_created_at
)

-- 사용자 통계
user_stats (
  user_id, total_debates, wins, losses, draws, win_rate,
  pro_debates, pro_wins, con_debates, con_wins,
  avg_score, highest_score, current_streak, best_win_streak
)

-- 실시간 방 (진행 중인 토론)
rooms (...), participants (...), messages (...)

-- 토론 주제
issues (...)
```

---

## 🔐 인증 시스템

### 카카오 OAuth 흐름

```
1. 사용자가 "카카오 로그인" 클릭
      │
      ▼
2. /api/auth/kakao → 카카오 인증 페이지로 리다이렉트
      │
      ▼
3. 카카오에서 로그인 후 콜백
      │
      ▼
4. /api/auth/kakao/callback
      ├── 토큰 교환
      ├── 사용자 정보 조회
      ├── Supabase users 테이블 upsert
      └── 세션 쿠키 생성
      │
      ▼
5. 홈으로 리다이렉트 (로그인 완료)
```

### 세션 관리

- **쿠키 기반**: `politi-log-session` (httpOnly, secure)
- **만료**: 7일
- **형식**: Base64 인코딩된 JSON

---

## 🔄 토론 기록 흐름

```
1. 토론 시작 (rooms 테이블)
      │
      ▼
2. 12단계 토론 진행 (messages 테이블)
      │
      ▼
3. 최종 판정 생성 (AI)
      │
      ▼
4. stage = 'ended' 전환
      │
      ▼
5. 자동 아카이브 트리거
      ├── debate_records 생성
      ├── debate_messages_archive 복사
      └── user_stats 업데이트
      │
      ▼
6. 마이페이지/관리자에서 조회 가능
```

---

## 🧩 기능 모듈 상태

| 모듈 | 상태 | 설명 |
|------|------|------|
| `debate` | 🟢 구현됨 | 토론 핵심 로직 |
| `ai-moderator` | 🟢 구현됨 | AI 분석 기능 |
| `auth` | 🟢 구현됨 | 카카오 로그인 |
| `admin` | 🟢 구현됨 | 이슈/토론 관리 |
| `user-stats` | 🟢 구현됨 | 전적/통계 시스템 |

---

## 📋 API 엔드포인트

### 인증 (/api/auth)

| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/auth/kakao | 카카오 로그인 시작 |
| GET | /api/auth/kakao/callback | OAuth 콜백 |
| GET | /api/auth/me | 현재 사용자 정보 |
| POST | /api/auth/logout | 로그아웃 |

### 토론 기록 (/api/debates)

| Method | Endpoint | 설명 |
|--------|----------|------|
| POST | /api/debates/archive | 토론 아카이브 |
| GET | /api/debates/archive | 기록 목록 조회 |
| GET | /api/debates/records/[id] | 기록 상세 조회 |
| DELETE | /api/debates/records/[id] | 기록 삭제 |

### 사용자 (/api/users)

| Method | Endpoint | 설명 |
|--------|----------|------|
| GET | /api/users/me/stats | 내 통계 |
| GET | /api/users/me/history | 내 토론 기록 |

---

## 🔒 보안 고려사항

### 구현됨

- ✅ 카카오 OAuth 인증
- ✅ httpOnly 세션 쿠키
- ✅ 프롬프트 인젝션 방지
- ✅ API 키 서버 사이드 보관
- ✅ 보호된 라우트 (로그인 필수)

### 계획됨

- 📋 JWT 토큰 (세션 대체)
- 📋 Rate limiting
- 📋 관리자 권한 검증 강화

---

## 🛠 기술 스택

| 기술 | 선택 이유 |
|------|----------|
| **Next.js 14** | SSR, App Router, API Routes 통합 |
| **Supabase** | PostgreSQL + 실시간 + 인증 올인원 |
| **Groq API** | 빠른 LLM 추론, Llama 3.1 지원 |
| **Kakao OAuth** | 한국 사용자 친화적 로그인 |
| **Zustand** | 간단한 상태 관리, React 친화적 |
| **Tailwind CSS** | 빠른 스타일링, 일관된 디자인 시스템 |
| **Framer Motion** | 부드러운 UI 애니메이션 |
| **Vercel** | Next.js 최적화 호스팅 |

---

*마지막 업데이트: 2024.12.26*
